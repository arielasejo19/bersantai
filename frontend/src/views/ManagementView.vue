...
const blankVillaType = () => ({ name: '', slug: '', description: '', isActive: true, dayTourOnly: false, defaultImageUrl: '', galleryUrls: [], nightlyPrice: 0, capacity: 2, bedroomCount: 1, status: 'active', availabilityStatus: 'available', amenities: [...defaultAmenities] });
...
async function saveVillaType() {
  ...
  try {
    ...
    if (selectedVillaType.value) await villaService.updateType(selectedVillaType.value.id, payload);
    else await villaService.createType(payload);
    openVillaTypeCreate();
