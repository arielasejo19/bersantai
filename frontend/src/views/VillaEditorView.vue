...
const form = reactive({ name: '', slug: '', location: '', villaTypeId: null, description: '', nightlyPrice: 0, capacity: 2, bedroomCount: 2, status: 'active', availabilityStatus: 'available', stayType: 'both', ownerUserId: null, amenities: [...defaultAmenities] });
...
async function load() {
  try {
    ...
    const [typeResult, villaResult] = await Promise.all([typeRequest, villaService.list()]);
    ...
    if (isEditing.value) {
      ...
      Object.assign(form, { ...villa, villaTypeId: villa.villaType?.id || null, ownerUserId: villa.owner?.id || null, amenities: (villa.amenities || []).map((item) => item.name || item) });
...
async function save() {
  ...
  try {
    const payload = { ...form, amenities: form.amenities.filter(Boolean), photos: media.value.filter((item) => !item.file).map((item, index) => ({ url: item.url, mediaType: item.mediaType, altText: item.altText || '', isThumbnail: Boolean(item.isThumbnail), sortOrder: index })), nightlyPrice: Number(form.nightlyPrice), capacity: Number(form.capacity), bedroomCount: Number(form.bedroomCount) };
    const result = isEditing.value ? await villaService.update(route.params.villaId, payload) : await villaService.create(payload);
    ...
    await villaService.update(villaId, { ...form, villaTypeId: form.villaTypeId ? Number(form.villaTypeId) : null, amenities: form.amenities.filter(Boolean), photos: finalPhotos, nightlyPrice: Number(form.nightlyPrice), capacity: Number(form.capacity), bedroomCount: Number(form.bedroomCount) });
...
<template>
  <main class="editor-page">
    <header class="editor-page-header"><button class="text-link" type="button" @click="router.push('/management')">← Back to management</button><img :src="'/icons/' + 'bersantai-logo.png'" alt="Bersantai" class="editor-page-logo"></header>
