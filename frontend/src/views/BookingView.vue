...
watch([currentStep, isAuthenticated], ([step, authenticated]) => {
  if (authenticated && user.value) {
    form.guestName = user.value.displayName || user.value.email || '';
    form.guestEmail = user.value.email || '';
  }
  if (step === 4 && authenticated && form.guestEmail && !verification.sent && !verification.loading) sendVerification();
});
...
async function submit() {
  ...
  try {
    ...
    const result = await villaService.book({ ...form, verificationToken: verification.token, villaId: usesVillaType ? null : Number(form.villaId), villaTypeId: usesVillaType ? Number(form.villaTypeId) : null, serviceIds: form.serviceIds.map(Number), menuItemIds: form.menuItemIds.map(Number), serviceQuantities, menuQuantities, guests: Number(form.guests), totalAmount: total.value });
  ...
  <main class="booking-page">
    ...
    <section v-else-if="!reservation" class="booking-page-layout">
      ...
      <form class="booking-page-form" @submit.prevent="submit">
        ...
        <div v-if="currentStep === 1" class="booking-columns"><label>Date<input v-model="form.checkIn" type="date" :min="today" required></label><label v-if="form.bookingKind === 'overnight'">Check-out<input v-model="form.checkOut" type="date" :min="form.checkIn || today" required></label></div>
