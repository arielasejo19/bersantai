...
const verification = reactive({ challengeId: '', token: '', code: '', sent: false, verified: false, loading: false, error: '', devCode: '', secondsRemaining: 0 });
...
const selectedAvailability = ref({ key: '', availableUnits: 0 });
const selectedAvailabilityLoading = ref(false);
const serviceQuantities = reactive({});
const menuQuantities = reactive({});
const currentStep = ref(1);
const steps = ['Stay & dates', 'Customize', 'Payment', 'Verify'];
const form = reactive({ villaId: '', villaTypeId: '', bookingKind: 'overnight', checkIn: '', checkOut: '', guests: 2, guestName: '', guestEmail: '', guestPhone: '', guestNote: '', serviceIds: [], menuItemIds: [], paymentMethod: 'card' });
const selectedVilla = computed(() => villas.value.find((villa) => String(villa.id) === String(form.villaId)) || null);
const availabilityKey = computed(() => [selectedVilla.value?.isVillaTypeOption ? 'type' : 'villa', selectedVilla.value?.id || '', form.bookingKind, form.checkIn, form.bookingKind === 'day_tour' ? form.checkIn : form.checkOut].join(':'));
const availabilityMessage = computed(() => {
  if (operatingMode.value !== 'hotel' || !form.checkIn || (form.bookingKind === 'overnight' && !form.checkOut)) return '';
  if (selectedAvailabilityLoading.value || selectedAvailability.value.key !== availabilityKey.value) return 'Checking available units...';
  return selectedAvailability.value.availableUnits > 0
    ? `${selectedAvailability.value.availableUnits} ${selectedAvailability.value.availableUnits === 1 ? 'unit' : 'units'} available for these dates`
...
const datesAvailable = computed(() => {
  ...
  return selectedAvailability.value.key === availabilityKey.value && selectedAvailability.value.availableUnits > 0;
});
...
function filterBookingVillas() {
  ...
  const visibleTypes = villaTypes.value
    ...
    .map((type) => {
      const gallery = [...new Set([type.defaultImageUrl, ...(type.galleryUrls || [])].filter(Boolean))];
      return { ...type, id: String(type.id), name: type.name, location: 'Bali Highlands', nightlyPrice: type.nightlyPrice, capacity: type.capacity, bedroomCount: type.bedroomCount, amenities: type.amenities || [], pricingRules: type.pricingRules || [], photos: gallery.map((url, index) => ({ url, mediaType: index === 0 ? type.defaultMediaType || 'image' : 'image' })), villaType: type, isVillaTypeOption: true };
    });
  ...
  villas.value = operatingMode.value === 'hotel' ? visibleTypes : visibleVillas;
...
watch(() => [form.villaId, form.villaTypeId, form.bookingKind, form.checkIn, form.checkOut], refreshSelectedAvailability);
watch(() => [form.villaId, form.villaTypeId, form.bookingKind, calendarMonth.value], refreshCalendarRates);
watch(() => form.guestEmail, () => { clearVerificationTimer(); verification.challengeId = ''; verification.token = ''; verification.code = ''; verification.sent = false; verification.verified = false; verification.error = ''; verification.devCode = ''; verification.secondsRemaining = 0; });
...
watch([currentStep, requiresVerification], ([step, verificationRequired]) => {
  if (user.value) {
    ...
    if (user.value.phone && !form.guestPhone) form.guestPhone = user.value.phone;
  }
...
function createCalendarMonth(date) {
  ...
  const days = Array.from({ length: daysInMonth }, (_, index) => {
    ...
    const checkoutDate = form.bookingKind === 'overnight' && form.checkIn && value > form.checkIn && !form.checkOut;
    return { value, day, rate: apiRate?.price ? apiRate : localPricingRate(value), disabled: value < today || (calendarUnavailableDates.value.includes(value) && !checkoutDate) };
  });
...
function selectCalendarDate(date) {
  ...
  const choosingCheckIn = !form.checkIn || form.checkOut || date <= form.checkIn;
  if (choosingCheckIn && calendarUnavailableDates.value.includes(date)) return;
  if (choosingCheckIn) { form.checkIn = date; form.checkOut = ''; return; }
  form.checkOut = date;
}
...
function reservationNights(booking) {
  if (booking.bookingKind === 'day_tour') return 0;
  const start = new Date(`${String(booking.checkIn).slice(0, 10)}T00:00:00Z`);
  const end = new Date(`${String(booking.checkOut).slice(0, 10)}T00:00:00Z`);
...
function startVerificationTimer(seconds) {
  ...
  const expiresAt = Date.now() + Number(seconds || 300) * 1000;
  const update = () => {
    verification.secondsRemaining = Math.max(0, Math.ceil((expiresAt - Date.now()) / 1000));
    if (verification.secondsRemaining === 0) {
      ...
      verification.error = 'Your verification code expired. Request a new one to continue.';
    }
  };
  update();
  verificationTimer = window.setInterval(update, 1000);
}
onUnmounted(clearVerificationTimer);
...
function calendarDateClass(day) {
  const checkoutCandidate = form.bookingKind === 'overnight' && form.checkIn && day.value > form.checkIn && !form.checkOut;
  return { 'calendar-day-selected': day.value === form.checkIn || day.value === form.checkOut, 'calendar-day-in-range': form.checkIn && form.checkOut && day.value > form.checkIn && day.value < form.checkOut, 'calendar-day-unavailable': calendarUnavailableDates.value.includes(day.value) && !checkoutCandidate, 'calendar-day-cheap': day.rate?.ruleType === 'weekday', 'calendar-day-expensive': ['weekend', 'holiday'].includes(day.rate?.ruleType) };
}
...
async function refreshSelectedAvailability() {
  if (!selectedVilla.value || !form.checkIn || (form.bookingKind === 'overnight' && !form.checkOut)) {
    selectedAvailability.value = { key: '', availableUnits: 0 };
    selectedAvailabilityLoading.value = false;
    return;
  }
  const key = availabilityKey.value;
  selectedAvailabilityLoading.value = true;
  try {
    const result = await villaService.availability({
      bookingKind: form.bookingKind,
      checkIn: form.checkIn,
      checkOut: form.bookingKind === 'day_tour' ? form.checkIn : form.checkOut,
      villaId: selectedVilla.value.isVillaTypeOption ? '' : form.villaId,
      villaTypeId: selectedVilla.value.isVillaTypeOption ? form.villaTypeId : ''
    });
    if (availabilityKey.value === key) selectedAvailability.value = { key, availableUnits: Number(result.availableUnits || 0) };
  } catch (_error) {
    if (availabilityKey.value === key) selectedAvailability.value = { key, availableUnits: 0 };
  } finally {
    if (availabilityKey.value === key) selectedAvailabilityLoading.value = false;
  }
}

async function load() {
  try {
    const configResult = await villaService.getPublicConfig();
    const [serviceResult, menuResult, typeResult, villaResult] = await Promise.all([
      serviceService.listPublic(), menuService.listPublic(), villaService.listPublicTypes(),
      configResult.config.operatingMode === 'hotel' ? Promise.resolve({ villas: [] }) : villaService.listPublic()
    ]);
    operatingMode.value = configResult.config.operatingMode;
    publicSite.value = configResult.config.publicSite || publicSite.value;
    if (['overnight', 'day_tour'].includes(String(route.query.bookingKind))) form.bookingKind = String(route.query.bookingKind);
    if (user.value) {
      form.guestName = user.value.displayName || user.value.email || '';
      form.guestEmail = user.value.email || '';
    }
...
async function sendVerification() {
  ...
  verification.verified = false;
  verification.devCode = '';
  verification.secondsRemaining = 0;
  clearVerificationTimer();
  error.value = '';
  try { const result = await authService.sendBookingVerification(form.guestEmail); Object.assign(verification, { challengeId: result.challengeId, sent: true, devCode: result.devCode || '' }); startVerificationTimer(result.expiresInSeconds); } catch (requestError) { verification.error = requestError.message; } finally { verification.loading = false; }
}
...
<template>
  <main class="booking-page">
    ...
    <section v-else-if="!reservation" class="booking-page-layout">
      <div class="booking-page-intro"><p class="eyebrow">{{ operatingMode === 'hotel' ? 'Hotel booking' : 'Private villa booking' }}</p><h1>Plan your<br><em>Bersantai stay.</em></h1><p>Select your stay, verify your email, and complete payment without creating an account.</p></div>
      <form class="booking-page-form" @submit.prevent="submit" @keydown="handleBookingKeydown">
        <div class="booking-form-heading"><p class="eyebrow">Complete your stay</p><h1>Plan your<br><em>Bersantai stay.</em></h1><p>Choose your dates, review the villa details, and send your booking request.</p></div><nav class="booking-stepper" aria-label="Booking steps"><button v-for="(step, index) in steps" :key="step" type="button" :class="{ active: currentStep === index + 1, complete: currentStep > index + 1 }" @click="index + 1 < currentStep ? currentStep = index + 1 : null"><span>{{ index + 1 }}</span>{{ step }}</button></nav>
        ...
        <section v-if="currentStep === 4" class="booking-verification-step">
          <div class="booking-verification-panel">
            <div><p class="eyebrow">Step 4 · Nearly there</p><h2>Verify your email</h2><p>{{ requiresVerification ? (isAuthenticated ? 'A six-digit code was sent to your account email.' : 'We will send a six-digit code to confirm this booking request.') : 'Your account email is verified.' }}</p></div>
            <template v-if="!isAuthenticated"><label>Your name<input v-model="form.guestName" autocomplete="name" required></label><label>Email address<input v-model="form.guestEmail" type="email" required></label></template>
            <label>Contact number<input v-model="form.guestPhone" type="tel" autocomplete="tel" minlength="5" maxlength="32" pattern="[+0-9() .-]{5,32}" required></label>
            <button v-if="requiresVerification && !verification.verified" class="button button-dark verification-send-button" :class="{ 'is-loading': verification.loading }" type="button" :disabled="verification.loading || !form.guestEmail" @click="sendVerification">{{ verification.loading ? 'Sending verification...' : (verification.sent ? 'Resend verification code' : 'Send verification code') }}</button>
            <small v-if="verification.devCode">Development code: {{ verification.devCode }}</small>
            <small v-if="verification.secondsRemaining">Code expires in {{ Math.floor(verification.secondsRemaining / 60) }}:{{ String(verification.secondsRemaining % 60).padStart(2, '0') }}</small>
            <label v-if="requiresVerification && !verification.verified && verification.sent">Verification code<input v-model="verification.code" inputmode="numeric" maxlength="6" autocomplete="one-time-code"><button class="button button-dark verification-check-button" :class="{ 'is-loading': verification.loading }" type="button" :disabled="verification.loading || verification.code.length !== 6" @click="verifyEmail">{{ verification.loading ? 'Verifying...' : 'Verify email' }}</button></label>
            <p v-if="!requiresVerification || verification.verified" class="booking-availability">Email verified for this booking.</p>
            <p v-if="verification.error" class="booking-error" role="alert">{{ verification.error }}</p>
          </div>
          <div class="booking-summary"><span>Estimated total</span><strong>{{ formatCurrency(total) }}</strong></div>
          <div class="booking-step-actions"><button class="button button-quiet" type="button" @click="previousStep">← Back</button><span>Step 4 of 4</span><button class="button button-sun hero-book-button" type="submit" :disabled="submitting || verification.loading || (requiresVerification && !verification.verified) || !datesAvailable">{{ submitting ? 'Creating reservation...' : 'Complete payment & booking' }} <span aria-hidden="true">↗</span></button></div>
        </section>
      </form>
    </section>
    <section v-else class="booking-confirmation booking-confirmation-page" aria-labelledby="booking-confirmation-title">
      ...
      <section class="confirmation-details" aria-label="Booking details">
        ...
        <div class="confirmation-detail-grid">
          ...
          <div><span>Guest</span><strong>{{ reservation.guestName }}</strong><small>{{ reservation.guestEmail }}</small></div>
          <div><span>Guests and stay</span><strong>{{ reservation.guests }} {{ reservation.guests === 1 ? 'guest' : 'guests' }}</strong><small>{{ reservation.bookingKind === 'day_tour' ? 'Day tour' : `${reservationNights(reservation)} ${reservationNights(reservation) === 1 ? 'night' : 'nights'}` }}</small></div>
