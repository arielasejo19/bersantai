...
function filterBookingVillas() {
  const visible = allVillas.value.filter((villa) => form.bookingKind === 'day_tour' || !villa.villaType?.dayTourOnly);
  const seen = new Set();
  villas.value = operatingMode.value === 'hotel' ? visible.filter((villa) => { const id = villa.villaType?.id; if (!id || seen.has(id)) return false; seen.add(id); return true; }) : visible;
  if (villas.value[0]) selectVilla(villas.value[0]);
}
function selectVilla(villa) { if (!villa) return; form.villaId = villa.id; form.villaTypeId = villa.villaType?.id || ''; }
watch(() => form.bookingKind, filterBookingVillas);

async function load() {
  try {
    const [villaResult, serviceResult, configResult] = await Promise.all([villaService.listPublic(), serviceService.listPublic(), villaService.getPublicConfig()]);
...
async function checkAvailability() {
  ...
  try {
    const result = await villaService.availability({ bookingKind: form.bookingKind, villaId: operatingMode.value === 'hotel' ? null : form.villaId, villaTypeId: operatingMode.value === 'hotel' ? form.villaTypeId : null, checkIn: form.checkIn, checkOut: form.bookingKind === 'day_tour' ? form.checkIn : form.checkOut });
...
async function submit() {
  ...
  try {
    const result = await villaService.book({ ...form, verificationToken: verification.token, villaId: operatingMode.value === 'hotel' ? null : Number(form.villaId), villaTypeId: operatingMode.value === 'hotel' ? Number(form.villaTypeId) : null, serviceIds: form.serviceIds.map(Number), guests: Number(form.guests), totalAmount: total.value });
  ...
  <main class="booking-page">
    ...
    <section v-else-if="!reservation" class="booking-page-layout">
      ...
      <form class="booking-page-form" @submit.prevent="submit">
        ...
        <label>{{ operatingMode === 'hotel' ? 'Villa Type' : 'Villa' }}<select v-model="form.villaId" required @change="selectVilla(selectedVilla)"><option v-for="villa in villas" :key="villa.id" :value="villa.id">{{ operatingMode === 'hotel' ? villa.villaType?.name : villa.name }}</option></select></label>
        ...
        <fieldset class="booking-services"><legend>Additional services</legend><label v-for="service in availableServices" :key="service.id" class="service-option"><input v-model="form.serviceIds" type="checkbox" :value="String(service.id)"><span>{{ service.title }}</span><strong>${{ service.price || 0 }}</strong></label></fieldset>
        ...
        <div class="booking-summary"><span>Estimated total</span><strong>${{ total }}</strong></div><button class="button button-sun" type="submit" :disabled="submitting || !verification.verified || availabilityMessage !== 'This selection is available.'">{{ submitting ? 'Creating reservation...' : 'Complete payment & booking' }} <span aria-hidden="true">↗</span></button>
      </form>
    </section>
