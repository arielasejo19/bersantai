import { addVillaAmenity, addVillaPhoto, assignVillaReceptionist, bookVilla, changeReservationStatus, createManagedVilla, editVilla, getAccounts, getPublicVillas, getVilla, getVillaReservations, getVillas, inviteStaff } from '../services/villaService.js';
...
export async function createBooking(request, response) { response.status(201).json({ reservation: await bookVilla(validateBody(bookingSchema, request.body)) }); }
